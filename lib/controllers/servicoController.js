import { servicoService, auditService } from '../services/index.js';
...
const servicoController = {
  ...
  async criar(req, res) {
    try {
      ...
      await auditService.registrar({ usuario: req.user, modulo: 'servicos', acao: 'INSERT', entidadeId: servico.id, descricao: `Serviço criado: ${req.body.nome || ''}`, ipAddress: req.ip });
  ...
  async atualizar(req, res) {
    try {
      ...
      await auditService.registrar({ usuario: req.user, modulo: 'servicos', acao: 'UPDATE', entidadeId: id, descricao: `Serviço atualizado: ${req.body.nome || id}`, ipAddress: req.ip });
  ...
  async excluir(req, res) {
    try {
      ...
      await auditService.registrar({ usuario: req.user, modulo: 'servicos', acao: 'DELETE', entidadeId: id, descricao: `Serviço inativado: ${id}`, ipAddress: req.ip });
