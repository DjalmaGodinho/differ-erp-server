...
const authController = {
  ...
  async criarUsuario(req, res) {
    try {
      ...
      await auditService.registrar({ usuario: req.user, modulo: 'usuarios', acao: 'INSERT', entidadeId: result?.id, descricao: `Usuário criado: ${email}`, ipAddress: req.ip });
  ...
  async atualizarRole(req, res) {
    try {
      ...
      await auditService.registrar({ usuario: req.user, modulo: 'usuarios', acao: 'UPDATE', entidadeId: id, descricao: `Role do usuário atualizado para: ${role}`, ipAddress: req.ip });
  ...
  async removerUsuario(req, res) {
    try {
      ...
      await auditService.registrar({ usuario: req.user, modulo: 'usuarios', acao: 'DELETE', entidadeId: id, descricao: `Usuário inativado: ${id}`, ipAddress: req.ip });
