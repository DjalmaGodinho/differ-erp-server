...
const pedidoController = {
  ...
  async efetivar(req, res) {
    try {
      ...
      // Efetivar o pedido (mudar tipo de Orçamento para Efetivado)
      const pedido = await pedidoService.efetivar(id);
      
      // Se deve ir direto para Concluído, atualiza o status
      if (irParaConcluido) {
        await pedidoService.atualizarStatus(id, 4); // 4 = Concluído
        
        // Gerar conta a receber
        const { contaReceberService } = await import('../services/index.js');
        await contaReceberService.gerarDoPedido(id, condicaoPagamentoId, formaPagamentoId);
      } else {
        // Vai para Em Produção
        await pedidoService.atualizarStatus(id, 3); // 3 = Em Produção
      }
