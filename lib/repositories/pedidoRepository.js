...
const pedidoRepository = {
  async findAll({ page = 1, pageSize = 20, search = null, tipoId = null, statusId = null, prioridadeId = null, clienteId = null, dataInicio = null, dataFim = null }) {
  ...
  async findById(id) {
    const pedidoResult = await query(`
      SELECT p.*, c.nome as cliente_nome, c.codigo as cliente_codigo, c.email as cliente_email,
    ...
    const materiaisResult = await query(`
      SELECT pm.*, m.descricao, m.codigo,
  ...
  async updateTipo(id, tipoId) {
    const result = await query(`
      UPDATE ${TABLE} SET tipo_id = $1, updated_at = NOW() WHERE id = $2 RETURNING *
    `, [tipoId, id]);
    return result.rows[0];
  },
