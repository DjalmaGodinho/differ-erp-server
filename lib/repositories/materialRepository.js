...
const materialRepository = {
  async findAll({ page = 1, pageSize = 20, search = null, tipoId = null, statusId = null, ativo = true }) {
    ...
    if (search) {
      whereClause += ` AND (descricao ILIKE $${paramIndex} OR codigo ILIKE $${paramIndex})`;
      params.push(`%${search}%`);
      paramIndex++;
    }
  ...
  async create(data) {
    const { 
      ...
      minimo, maximo, local, status_id = 1, valor_unitario, ncm, 
    ...
    const result = await query(`
      INSERT INTO ${TABLE} (
        ...
        minimo, maximo, local, status_id, valor_unitario, ncm, 
  ...
  async update(id, data) {
    const { 
      descricao, tipo_id, unidade_id, minimo, maximo, local, 
