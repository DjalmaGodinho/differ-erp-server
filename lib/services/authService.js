...
const authService = {
  async login(email, senha) {
    ...
    let role = 'usuario';
    console.log('UserID do Supabase:', userId);
    console.log('Email do Supabase:', result.user?.email);
    if (userId) {
      try {
        let profile = await usuarioRepository.findById(userId);
        console.log('Profile por ID:', profile);
        // Fallback: buscar por email se não encontrar por ID
        if (!profile && result.user?.email) {
          profile = await usuarioRepository.findByEmail(result.user.email);
          console.log('Profile por email:', profile);
        }
        if (profile) {
          role = profile.role;
          console.log('Role final:', role);
        } else {
          console.log('Nenhum profile encontrado, usando role default: usuario');
        }
      } catch (e) {
