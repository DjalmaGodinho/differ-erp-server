...
function infoGrid(doc, items, y) {
  ...
  items.forEach((item, i) => {
    ...
    const x = ML + col * colW;
    const iy = y + row * 28;
    doc.fontSize(7).fillColor(LGRAY).font('Helvetica').text(item.label, x, iy, { width: colW - 4 });
    doc.fontSize(9).fillColor('#222222').font('Helvetica-Bold').text(item.value || '—', x, iy + 9, { width: colW - 4 });
  });
